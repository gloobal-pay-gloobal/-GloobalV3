...
function PayLaterScreen({ onClose, ccy, paylaterAvailable, paylaterLimit, totalAssets, paylaterDue, paylaterReceiving, paylaterSending, onViewAssets, onPayNow, toast }) {
  ...
  ><ArrowLeft2 size={18} color={T.ink} /></button><span style={{ fontSize: 16, fontWeight: 800, color: T.ink, fontFamily: T.fontDisplay }}><GloobalWordmark suffix=" PayLater" /></span></div><div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch", padding: "6px 18px 30px", display: "flex", flexDirection: "column", gap: 16 }}>{
    /* Balance */
  ...
  ><span style={{ fontSize: 11, fontWeight: 700, color: T.accent }}>Your limit = your current assets ({ccy}{totalAssets.toFixed(2)})</span><ChevronRight2 size={13} color={T.accent} /></button></div>{
    /* Pending dues */
  }<div style={{ borderRadius: T.radiusLg, background: T.surface, boxShadow: T.shadowCard, padding: "18px" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}><span><div style={{ fontSize: 12, fontWeight: 600, color: T.inkSoft }}>Pending dues</div><div style={{ fontSize: 20, fontWeight: 800, color: paylaterDue > 0 ? T.negative : T.positive, fontFamily: T.fontDisplay, marginTop: 3 }}>{ccy}{paylaterDue.toFixed(2)}</div><div style={{ fontSize: 11.5, color: T.inkFaint, marginTop: 2 }}>{paylaterDue > 0 ? "Due Aug 1" : "Nothing due \u2014 all clear"}</div></span>{paylaterDue > 0 && <button
    onClick={onPayNow}
    className="v2-tap"
    style={{
      border: "none",
  ...
  }<PayLaterLedger title="Receiving" rows={paylaterReceiving} direction="in" ccy={ccy} /><PayLaterLedger title="Sending" rows={paylaterSending} direction="out" ccy={ccy} /></div>{
